...
print("bOttomansOverride is"..tostring(bOttomansOverride))
local bScythiaOverride = MapConfiguration.GetValue("ScythiaTSL")
print("bScythiaOverride is"..tostring(bScythiaOverride))
local bPhoeniciaOverride = MapConfiguration.GetValue("PhoeniciaTSL")
print("bPhoeniciaOverride is"..tostring(bPhoeniciaOverride))
local bSumeriaOverride = MapConfiguration.GetValue("SumeriaTSL")
print("bSumeriaOverride is"..tostring(bSumeriaOverride))
local bHungaryOverride = MapConfiguration.GetValue("HungaryTSL")
print("bHungaryOverride is"..tostring(bHungaryOverride))
			...
			if pPlot then
				...
				if featureTypeName == "FEATURE_LAKE_VICTORIA" then
					...
					elseif(mapName == "EqualAreaEarth") then
						...
						-- table.insert(plotsList, { Plot = pPlot2, Terrain = terrainType })
						-- table.insert(plotsList, { Plot = Map.GetAdjacentPlot(x, y, DirectionTypes.DIRECTION_SOUTHWEST), Terrain = terrainType })
						-- table.insert(plotsList, { Plot = Map.GetAdjacentPlot(pPlot2:GetX(), pPlot2:GetY(), DirectionTypes.DIRECTION_SOUTHWEST), Terrain = terrainType })						
					else
						local terrainType = g_TERRAIN_TYPE_COAST
				...
				if bPlaced then
					...
					if mapName == "Earth128x80" or mapName == "EqualAreaEarth" then
						if featureTypeName == "FEATURE_CERRO_DE_POTOSI" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									...
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											TerrainBuilder.SetTerrainType( otherPlot, g_TERRAIN_TYPE_PLAINS_MOUNTAIN )
										end
									end
								end
							end
						...
						if featureTypeName == "FEATURE_CLIFFS_DOVER" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									...
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											TerrainBuilder.SetTerrainType( otherPlot, g_TERRAIN_TYPE_GRASS_HILLS )
										end
									end
								end
							end
						...
						if featureTypeName == "FEATURE_EVEREST" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									...
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											TerrainBuilder.SetTerrainType( otherPlot, g_TERRAIN_TYPE_TUNDRA_MOUNTAIN )
										end
									end
								end
							end
						end
						if featureTypeName == "FEATURE_CHOCOLATEHILLS" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_SUK_FUJI" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											TerrainBuilder.SetTerrainType( otherPlot, g_TERRAIN_TYPE_GRASS_MOUNTAIN )
										end
									end
								end
							end
						end
						if featureTypeName == "FEATURE_GRAND_MESA" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_KAILASH" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_KILIMANJARO" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_KRAKATOA" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
									...
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											...
											for direction = 0, DirectionTypes.NUM_DIRECTION_TYPES - 1, 1 do
												adjacentPlot = Map.GetAdjacentPlot(otherPlot:GetX(), otherPlot:GetY(), direction);
												if (adjacentPlot ~= nil) then
													if not adjacentPlot:IsNaturalWonder() and adjacentPlot:IsMountain() then
						...
						if featureTypeName == "FEATURE_MATTERHORN" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_SINAI" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
										if otherPlot:GetFeatureType() == eFeatureType then
											TerrainBuilder.SetTerrainType( otherPlot, g_TERRAIN_TYPE_DESERT_MOUNTAIN )
										end
									end
								end
							end
						end
						if featureTypeName == "FEATURE_PAITITI" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_RORAIMA" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_SRI_PADA" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_VESUVIUS" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_WULINGYUAN" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
						...
						if featureTypeName == "FEATURE_ZHANGYE_DANXIA" then				
							for dx = -3, 3 do
								for dy = -3,3 do
									local otherPlot = Map.GetPlotXY(plotX, plotY, dx, dy, 3)
									if(otherPlot) then
		...
		if Data.MapName == mapName  then
			if Data.Region == region then
				...
				if mapName == "Earth128x80" then
					...
					width		= g_ReferenceWidthRatio * Data.Width
					height		= g_ReferenceHeightRatio * Data.Height				
				end
					...
					if player:IsMajor() then
						if IsSafeStartingDistance(plot, true, player:IsHuman()) then
							...
							if ((mapName == "Earth128x80") or (mapName == "EqualAreaEarth") or (mapName == "Earth128x80_Alt")) then
								...
								if CivilizationTypeName == "CIVILIZATION_ENGLAND" then
									player:SetStartingPlot(plot)
									print ("Used original starting plot anyway for England")
								end
						...
						if ((mapName == "Earth128x80") or (mapName == "Earth128x80_Alt")) then
							if CivilizationTypeName == "CIVILIZATION_OTTOMAN" and bOttomansOverride then
								...
								print ("Ottoman Starting Plot has been moved to Hattusa's starting plot")
							end
							if CivilizationTypeName == "CIVILIZATION_SCYTHIA" and bScythiaOverride then
								local asiaPlot = Map.GetPlot(91, 59)
								player:SetStartingPlot(asiaPlot)
								print ("Scythian Starting Plot has been moved to Central Asia")
							end
							if CivilizationTypeName == "CIVILIZATION_PHOENICIA" and bPhoeniciaOverride then
								local tyrePlot = Map.GetPlot(76, 51)
								player:SetStartingPlot(tyrePlot)
								print ("Phoenician Starting Plot has been moved to Tyre")
							end
							if CivilizationTypeName == "CIVILIZATION_SUMERIA" and bSumeriaOverride then
								local babylonPlot = Map.GetPlot(80, 52)
								player:SetStartingPlot(babylonPlot)
								print ("Sumerian Starting Plot has been moved to Babylon")
							end
							-- if CivilizationTypeName == "CIVILIZATION_HUNGARY" and bHungaryOverride then
								-- local uralPlot = Map.GetPlot(83, 73)
								-- player:SetStartingPlot(uralPlot)
								-- print ("Hungarian Starting Plot has been moved to the Urals")
							-- end							
						end
						--totalslacker: Special override options for EqualAreaEarth
						if mapName == "EqualAreaEarth" then
							if CivilizationTypeName == "CIVILIZATION_OTTOMAN" and bOttomansOverride then
								local hattusaPlot = Map.GetPlot(77, 63)
								player:SetStartingPlot(hattusaPlot)
								print ("Ottoman Starting Plot has been moved to Hattusa's starting plot")
							end
							if CivilizationTypeName == "CIVILIZATION_PHOENICIA" and bPhoeniciaOverride then
								local tyrePlot = Map.GetPlot(76, 59)
								player:SetStartingPlot(tyrePlot)
								print ("Phoenician Starting Plot has been moved to Tyre")
							end
							if CivilizationTypeName == "CIVILIZATION_SCYTHIA" and bScythiaOverride then
								local asiaPlot = Map.GetPlot(90, 67)
								player:SetStartingPlot(asiaPlot)
								print ("Scythian Starting Plot has been moved to Central Asia")
							end
							if CivilizationTypeName == "CIVILIZATION_SUMERIA" and bSumeriaOverride then
								local babylonPlot = Map.GetPlot(80, 59)
								player:SetStartingPlot(babylonPlot)
								print ("Sumerian Starting Plot has been moved to Babylon")
							end	
							if CivilizationTypeName == "CIVILIZATION_HUNGARY" and bHungaryOverride then
						...
						if IsSafeStartingDistance(plot, false, false) then
							...
							if ((mapName == "Earth128x80") or (mapName == "EqualAreaEarth") or (mapName == "Earth128x80_Alt")) then
								...
								if CivilizationTypeName == "CIVILIZATION_WOLIN" then
									player:SetStartingPlot(plot)
									print ("Used original starting plot anyway for Wolin")
								end
								if CivilizationTypeName == "CIVILIZATION_BRUSSELS" then
									player:SetStartingPlot(plot)
									print ("Used original starting plot anyway for Brussels")
								end
								if CivilizationTypeName == "CIVILIZATION_VILNIUS" then
									player:SetStartingPlot(plot)
									print ("Used original starting plot anyway for Vilnius")
								end
							end
						end
					end
